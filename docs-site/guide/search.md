...
- `src/clible-web/App.tsx` calls `bibleRepository.search(query, translationId)` (default `-n 50` on the CLI for a smaller JSON payload).
- `src/clible-web/repositories/bibleRepository.ts` builds the same argument string the CLI would use, e.g. `"mountain" -t web`, and requests:
...
## 2. Express bridge (`src/clible-web/server.ts`)
...
- `src/clible/commands/search.py` loads `VerseService` and calls `search_text(...)`.
- For **`--json`**:
...
- `VerseService.search_text` (`src/clible/services/verse_service.py`) resolves scope (bible / book / chapter / …) and calls `VerseRepo.search_text` (`src/clible/db/repositories/verse_repo.py`).
...
Book IDs in `top_books` (e.g. `ROM`) are resolved to full names (e.g. `Romans`) on the client using `src/clible-web/utils/bookNames.ts`, which builds a lookup map from the static `src/clible-web/data/bible_structure.json` bundled with the web app at build time. No extra server round-trip or API call is needed.
