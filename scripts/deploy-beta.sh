...
# Get secrets
if [ -z "${GEMINI_API_KEY_FOR_BETA_TESTERS:-}" ]; then
    read -sp "Enter Gemini API Key for Beta Testers: " GEMINI_API_KEY_FOR_BETA_TESTERS
    echo ""
    export GEMINI_API_KEY_FOR_BETA_TESTERS
fi
...
gcloud run deploy clible-web \
  ...
  --concurrency=80 \
  --timeout=60 \
  --set-env-vars="GEMINI_API_KEY_FOR_BETA_TESTERS=$GEMINI_API_KEY_FOR_BETA_TESTERS,SESSION_SECRET=$SESSION_SECRET,NODE_ENV=production,MAX_REQUESTS_PER_HOUR=$MAX_REQUESTS_PER_HOUR" \
  --project=$GCP_PROJECT_ID
...
GEMINI_API_KEY_FOR_BETA_TESTERS=$GEMINI_API_KEY_FOR_BETA_TESTERS
